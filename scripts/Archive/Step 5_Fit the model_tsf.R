...
fire_modelling <- data %>%
  ...
  dplyr::select(
    ...
    fuel_management_zones, distance_roads, tsf
  ) %>%
...
brt_full_tsf <- fit_iterative_brt(data = fire_modelling_for_brt, 0.05, seed = 123)

# Save it for future use
saveRDS(brt_full_tsf, "F:/vic_fire_mapping/output_data/lr_0.05_fire_model_tsf.RDS")

# Add residuals to dataset
fire_modelling$residuals <- brt_full_tsf$residuals

# 1. Read the model back into R
brt_full_tsf <- readRDS("F:/vic_fire_mapping/output_data/lr_0.05_fire_model_tsf.RDS")
...
save(brt_full_tsf, file = "F:/vic_fire_mapping/output_data/fire_model_tsf.RData")
...
fire_modelling_rac_ready <- fire_modelling_rac %>%
  dplyr::select(x, y, burnt, bio5, spei12_mean, ffdi_95_days, spei24_mean, bio18,
         kbdi_95_days, twi, local_refuges, broad_refuges, thunderstorm_days,
         fuel_management_zones, distance_roads, tsf, rac) %>%
  mutate(fuel_management_zones = as.factor(fuel_management_zones)) %>%
  drop_na()

write_csv(fire_modelling_rac_ready,
           "F:/vic_fire_mapping/output_data/fire_modelling_rac_ready_tsf.csv")
...
# -----------------------------------------------------------------------------
#fire_modelling_rac_ready <- read.csv("F:/vic_fire_mapping/output_data/fire_modelling_rac_ready_tsf.csv")
...
saveRDS(brt_full_rac_extra,
        "F:/vic_fire_mapping/output_data/brt_full_rac_tsf.rds")
