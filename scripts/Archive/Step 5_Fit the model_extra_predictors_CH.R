...
brt_full_extra <- fit_iterative_brt(data = fire_modelling, 0.025, seed = 123)

# Save it for future use
saveRDS(brt_full_extra, "F:/vic_fire_mapping/output_data/lr_0.025_fire_model_EXTRA_PREDICTORS.RDS")

# Add residuals to dataset
fire_modelling$residuals <- brt_full_extra$residuals

# 1. Read the model back into R
brt_full_extra <- readRDS("F:/vic_fire_mapping/output_data/lr_0.025_fire_model_EXTRA_PREDICTORS.RDS")

# 2. Inspect it (optional)
print(brt_full_extra)
str(brt_full_extra)

# 3. (Optionally) save it in your .RData workspace so it’s automatically loaded next time
save(fire_model, file = "F:/vic_fire_mapping/output_data/fire_model_EXTRA_PREDICTORS.RData")
...
fire_modelling_rac_ready <- fire_modelling_rac %>%
  select(x, y, burnt, bio5, spei12_mean, ffdi_95_days, spei24_mean, bio18,
         kbdi_95_days, twi, local_refuges, broad_refuges, thunderstorm_days,
         fuel_management_zones, distance_roads, rac, year) %>%
  mutate(fuel_management_zones = as.factor(fuel_management_zones)) %>%
  drop_na()

write_csv(fire_modelling_rac_ready,
          "F:/vic_fire_mapping/output_data/fire_modelling_rac_ready_EXTRA_PREDICTORS.csv")
...
# -----------------------------------------------------------------------------
fire_modelling_rac_ready <- read.csv("F:/vic_fire_mapping/output_data/fire_modelling_rac_ready_EXTRA_PREDICTORS.csv")
...
saveRDS(brt_full_rac_extra,
        "F:/vic_fire_mapping/output_data/brt_full_rac_EXTRA_PREDICTORS.rds")
