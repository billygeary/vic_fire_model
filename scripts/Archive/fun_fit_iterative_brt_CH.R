...
# Function modified version of function by Saiorse Kelleher 
...
fit_iterative_brt = function(data.in, initialLearningRate, seed, max_trees = 10000){
    ...
    withr::with_seed(seed, {
      modelBRT <- 
        dismo::gbm.step(data = data.in,
                        ...
                        n.folds = length(unique(data.in$spatial_temporal_folds)),
  ...
  if (is.null(modelBRT)) {
    ...
    # Save the model
    filename.out <- file.path(output_dir, paste0("lr_", learningRate, "_fire_model.RDS"))
